...
class count_dataset(Dataset):
    def __init__(self, path, classes, img_size=640):
        ...
        for line in annoFile:
            ...
            gt_boxes.append(
                [float(bbox[0]), float(bbox[1]), float(bbox[2]), float(bbox[3]), float(bbox[4]),
                 float(bbox[5]), float(bbox[6]), float(bbox[7]),  bbox[8]])
        return gt_boxes
    ...
    def __next__(self):
        ...
        label_path = path.replace('.png', '.txt').replace('images', 'labelTxt')
        annoFile = open(label_path, 'r')
        gt_boxes = np.asarray(self.read_gt_bbox(annoFile))
        gt_boxes = np.delete(gt_boxes, np.where((gt_boxes[:, 8] != self.classes)), 0)
        target_count = gt_boxes.shape[0]
        ...
        return path, img, img0, self.cap, target_count
